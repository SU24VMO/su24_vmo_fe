...
import { CREATECAMPAIGN, GETREQUESTCAMPAIGNTOUPDATE, UPDATECAMPAIGNVOLUNTEER } from "../../api/apiConstants";
...
export default function UpdateCampaignVolunteerPage() {
    const { id } = useParams();
    ...
    const updateCampaign = async (data, resetForm) => {
        ...
        try {
            const response = await axiosPrivate.post(UPDATECAMPAIGNVOLUNTEER + `?createCampaignRequestId=${id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
