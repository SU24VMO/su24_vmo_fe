...
async function getData(cancelToken, user,  pageSize, pageNo, sortConfig,campaignName, setLoading) {
  ...
  try {
    const response = await axiosPrivate.get(GETALLPHASE123BYOM + `${user.organization_manager_id}/statement-phase/processing-status?pageSize=${pageSize}&pageNo=${pageNo}&orderBy=${sortConfig.orderByDirection}&orderByProperty=${sortConfig.orderByProperty}&campaignName=${campaignName}`, {
