...
import { PUTSTATEMENTPHASE } from "../../../../api/apiConstants";
...
const ConfirmDialog = ({ isOpen, onOpenChange, row, onSubmitSuccess }) => {
  ...
  const onUpdateStatusPhase = async (statementPhaseId) => {
    setLoading(true)

    try {
      const response = await axiosPrivate.put(PUTSTATEMENTPHASE, {
        statementPhaseId: statementPhaseId,
        isEnd: true,
