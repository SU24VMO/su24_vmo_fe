...
import { CREATECAMPAIGN, GETREQUESTCAMPAIGNTOUPDATE, UPDATECAMPAIGNTIER2VOLUNTEER } from "../../api/apiConstants";
...
export default function UpdateCampaignVolunteerTier2Page() {
    const { id } = useParams();
    ...
    const updateCampaign = async (data, resetForm, setSubmitting) => {
        ...
        try {
            const response = await axiosPrivate.put(UPDATECAMPAIGNTIER2VOLUNTEER + `?createCampaignRequestId=${id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
